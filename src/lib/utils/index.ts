...
// generateAvatarFallback delegates to the title-aware initials helper so a
// titled name like "Dr. E. Mwape" produces "EM" rather than "DE".
import { userInitials } from "./names";

export function generateAvatarFallback(string: string) {
  return userInitials({ name: string });
}

export function getAvatarSrc(name: string) {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(
    name,
  )}&background=276a57&color=ffffff&size=128`;
}
...
export function generateMeta({
  ...
  return {
    ...
    metadataBase: new URL(`https://edusync.app/`),
    alternates: {
...
// getInitials returns initials from a single display string. Honorifics
